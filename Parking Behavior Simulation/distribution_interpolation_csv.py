...
# 读取数据
file_path = "/Users/fujunhan/Desktop/RA_traffic simulation/my_research_related_materials/plan2.23/2020Melbourne_clean.csv"
...
# 图 1: State = 0 (空闲状态)
state_0 = df[(df['State'] == 0) & (df['lastingtime'] <= time_threshold)]
...
# 图 2: State = 1 (占用状态)
state_1 = df[(df['State'] == 1) & (df['lastingtime'] <= time_threshold)]
...
axs[0].set_xlabel('Time (seconds)', fontsize=AXIS_LABEL_SIZE, fontweight='bold')
axs[0].set_ylabel('Survival Probability', fontsize=AXIS_LABEL_SIZE, fontweight='bold')
axs[0].set_title('Idle State Survival Curve', fontsize=TITLE_SIZE, fontweight='bold')
...
axs[1].set_xlabel('Time (seconds)', fontsize=AXIS_LABEL_SIZE, fontweight='bold')
axs[1].set_ylabel('Survival Probability', fontsize=AXIS_LABEL_SIZE, fontweight='bold')
axs[1].set_title('Occupied State Survival Curve', fontsize=TITLE_SIZE, fontweight='bold')
